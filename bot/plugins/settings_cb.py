...
async def settings_cb(c, m):
    ...
    if typ == "af":
        ...
        alert_text = "Successfully changed screenshot upload mode! <b>~@world_wide_movies</b>"

    elif typ == "wm":
        watermark_text = await db.get_watermark_text(chat_id)
        if watermark_text:
            await db.update_watermark_text(chat_id)
            alert_text = "Successfully removed watermark text. <b>~@world_wide_movies</b>"
        else:
        ...
        alert_text = "Successfully changed screenshot generation mode <b>~@world_wide_movies</b>"
...
async def _(c, m):
    await m.answer("😂 press the other button 😂 <b>~@world_wide_movies</b>")
